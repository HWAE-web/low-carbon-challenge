              ...
              <TitleCol>
                ...
                <R2>
                  <TitleRow>
                    <C1>
                      <Desc>
                        집 근처 가까운 곳에서 저탄소 챌린지를 실천할 수 있는
                        장소들을 찾아보세요!
                      </Desc>
                    </C1>
                    <C2>
                      ...
                      >
                        ...
                        <SelectOptions show={showOptions}>
                          ...
                          <Option
                            onClick={() => (
                              setCurrentValue("소품샵 / 생활용품"),
                              setIsCafeOpen(false),
                              setIsRestOpen(false),
                              setIsShopOpen(true)
                            )}
                          >
                            <OptionImg src={shop} alt="shop" />
                            소품샵 / 생활용품
                          </Option>
                        </SelectOptions>
                      </SelectBox>
                    ...
                    <C3>
                      <MapSearchArea>
                        ...
                        <DetailInfo>
                          기억하고 싶은 장소의 마커를 클릭해 보세요!
                        </DetailInfo>
                        <ClearBtn
                          onClick={() => {
                            dispatch(
                              clearState({
                                id: state.detail[0].id,
                                name: state.detail[0].name,
                                address: state.detail[0].address,
                                latlng: state.detail[0].latlng,
                              })
                            );
                          }}
                        >
                  ...
                  <DetailWrap>
                    {state.detail.map((a, i) => (
                      <DetailArea key={i}>
                        ...
                        <Dot src={dot} alt="dot" />
                        <PostLink2 as="a" href={state.detail[i].href}>
                          상세정보
                        </PostLink2>
                      </DetailArea>
                    ))}
...
export function MapAPI(props) {
  ...
  const [points, setPoints] = useState([{ lat: 37.5618588, lng: 126.9468339 }]);
            ...
            props.cafeData.map((a, index) => (
              <MapMarker
                ...
                onClick={() => {
                  dispatch(
                    addState({
                      id: props.cafeData[index].id,
                      name: props.cafeData[index].name,
                      address: props.cafeData[index].address,
                      latlng: props.cafeData[index].latlng,
            ...
            props.restaurantData.map((a, index) => (
              <MapMarker
                ...
                onClick={() => {
                  dispatch(
                    addState({
                      id: props.restaurantData[index].id,
                      name: props.restaurantData[index].name,
                      address: props.restaurantData[index].address,
                      latlng: props.restaurantData[index].latlng,
            ...
            props.shopData.map((a, index) => (
              <MapMarker
                ...
                onClick={() => {
                  dispatch(
                    addState({
                      id: props.shopData[index].id,
                      name: props.shopData[index].name,
                      address: props.shopData[index].address,
                      latlng: props.shopData[index].latlng,
                ...
                <div className="customoverlay1">
                  ...
                  >
                    <span className="title">
                      {props.restaurantData[index].name}
                    </span>
                  </a>
                </div>
              </CustomOverlayMap>
